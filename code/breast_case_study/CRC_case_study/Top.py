...
for i in range(0, cell_type + 1):
    for j in range(0, cell_type + 1):
        if (i == 0 and j != 0) or (i != 0 and j == 0):
            list1 = pd.read_csv(cancer + "mult_list" + str(i) + str(j) + ".csv",header=None, index_col=None)
            list2 = pd.read_csv(cancer + "cell_exp_list" + str(i) + str(j) + ".csv",header=None, index_col=None)
...
fig = plt.figure()
sns_plot = sns.heatmap(result, cmap='Blues', xticklabels = xtick,    # Blues OrRd
                       yticklabels = ytick,  linewidths=0.8  #, linecolor= 'black'
                       )
